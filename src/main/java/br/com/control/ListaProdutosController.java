...
import br.com.control.ListaClientesController.ClienteTabela;
...
public class ListaProdutosController implements Initializable {
	...
	public void alterarProdutos() {
		...
		pdao.alterar(codigoAlt, nomeDigitado, valorDigitado);
		tabelaProd.setItems(ObterlistaProdutos());
	...
	private ObservableList<ProdutoTabela> ObterlistaProdutos() {
		...
		for(int i=0; i<listaDoBanco.size();i++) {
			...
			ProdutoTabela prodT = new ProdutoTabela(prod.getId_produto(), prod.getNomeProduto(), prod.getValorProduto());
			list.add(prodT);
		}
		return list;
		}
